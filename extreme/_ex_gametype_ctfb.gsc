		...
		if(isPlayer(other) && isAlive(other) && (other.pers["team"] != "spectator"))
		{
			...
			if(other.pers["team"] == self.team) // Touched by team
			{
				if(self.atbase)
				{
					if(isDefined(other.flag) && (other.pers["team"] != other.flag.team)) // Captured flag
					{
						...
						other.pers["flagcap"]++;
						other thread [[level.ex_scorePlayer]](level.ex_ctfbpoints_playercf, "special");
				...
				else // Picked up own flag
				{
					...
					if(!isDefined(other.ownflagDropped)) other thread [[level.ex_scorePlayer]](level.ex_ctfbpoints_playerpf, "special");
				}
			}
			else if(other.pers["team"] != self.team) // Touched by enemy
			{
				...
				if(self.atbase) // Stolen flag
				{
					...
					other thread [[level.ex_scorePlayer]](level.ex_ctfbpoints_playersf, "special");
				}
				else // Picked up flag
				{
					lpselfnum = other getEntityNumber();
					...
					if(!isDefined(other.enemyflagDropped)) other thread [[level.ex_scorePlayer]](level.ex_ctfbpoints_playertf, "special");
				}
...
checkBaseHomeOwnFlag(flag)
{
	...
	while(isDefined(flag))
	{
		...
		if(isDefined(flag) && (self.sessionstate == "playing") && (distance(flag.basemodel.origin, self.origin) < 50))
		{
			...
			self.pers["flagret"]++;
			self thread [[level.ex_scorePlayer]](level.ex_ctfbpoints_playerrf, "special");
			break;
		}
	}	
}
			...
			if(isDefined(dropspot))
			{
				if(isDefined(self.enemyflagAttached))
				{
					...
					if(!isDefined(self.ownflagAttached)) break;
						else while(self useButtonPressed()) wait( level.ex_fps_frame );
				}
